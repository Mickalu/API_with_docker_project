...
## Conclusion 
Pour faire un projet, le choix des technologies est très important. Ici, nous avons utilisé FAST API pour faire l'api car cela est simple à implémenter pour les petits projets et cela est intuitif. Toutes les personnes travaillant sur le projet peuvent faire leurs propres pages sans que cela impacte les autres. Il est aussi possible de faire des API avec **Flask** en Python, mais cela est moins intuitif et demande plus de travail. Pour un petit projet comme le nôtre, FAST API est largement suffisant. 

Nous avons aussi travaillé avec Docker, cela va permettre de créer un environnement de travail avec tout d'installé directement sur l'ordinateur. Cela est très intéressant car si des personnes ne travaillent pas sur le même système d'exploitation, il peut y avoir des bug, alors que ici non. Le seul défaut est que cela demande à ce que les personnes travaillant sur le projet soient organisées. Pour nous, il est assez simple de travailler comme cela vu que nous sommes que deux sur le projet, en plus le projet n'est pas très volumineux donc cela est plus simple. 

Nous nous sommes lancé directement dans les technologies simples et que nous connaissions mais il aurait pu être intéressant de s'intéresser aux autres technologies au départ et voir les avantages et inconvénients de chacun pour faire le meilleur choix. Ici, le projet n'était pas très grand et donc les outils étaient adapté.  